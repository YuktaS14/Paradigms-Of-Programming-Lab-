...
fun unify (tel: telescope ) (sEQt: equation) : telescope=
    case sEQt of
	(Var x , t) => AtomMap.empty
      | (s,  Var y) => AtomMap.empty
      | (Apply (f,fargs), Apply (g,gargs)) => AtomMap.empty
     ...
     fun changeVal (tel: telescope, eq: eq) = case eq of
						...
						| (term.Var(x)) => (let val y = Atom.atom(V.toString(x))
								...
								(case AtomMap.find(tel, y) of
								   SOME vl => changeVal (tel,vl)
								  | NONE => term.Var(x))
							       end
							      )
								  
						| (term.Apply(f, ls)) => let fun check(t,[]) = []
									  | check (t, x::xs) = (changeVal(t,x):: check(t,xs))
...
fun unify (tel: telescope ) (sEQt: equation) : telescope option =
    case sEQt of
	(Var x , t) => unifyVar tel x t
      | (s,  Var y) => unifyVar tel y t
      | (Apply (f,fargs), Apply (g,gargs)) => SOME AtomMap.empty
...
(*
telescope option
fresh variable-type inference 
structure VarMap RedBlackMapFn(V)

structure IntVar : VAR = struct
var = int
